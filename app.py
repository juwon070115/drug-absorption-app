...
- **TPSA (극성 표면적)**  
  극성이 크면 수용성은 높으나 세포막 통과는 어려워집니다.

- **pKa (이온화 상수)**  
  약물의 이온화 상태는 pH에 따라 변하며, 이온화된 상태는 투과가 어렵습니다.

- **pH (위장 내 pH)**  
  공복과 식후 상태에 따라 pH가 달라지며, 이는 이온화 정도와 흡수 위치에 큰 영향을 미칩니다.
...
# ----------------------- 앱 UI -----------------------
st.title("💊 약물 흡수 위치 예측기 (공복/식후 반영 + pH 자동 적용)")
...
# 사용자 입력
mw = st.number_input("📦 분자량 (Molecular Weight)", 50.0, 1000.0, 300.0)
logp = st.number_input("💧 LogP (지용성)", -5.0, 10.0, 2.5)
tpsa = st.number_input("🧪 TPSA (극성 표면적)", 0.0, 200.0, 40.0)
pka = st.number_input("🧬 pKa (이온화 상수)", 0.0, 14.0, 5.5)

# 자동 설정된 pH 표시 (수정 불가)
st.info(f"🌡 자동 적용된 위장 pH (식사 상태 기준): **{default_pH:.2f}**")
ph = default_pH
...
if st.button("📊 예측하기"):
    ...
    st.success(f"✅ 이 약물은 **'{site}'**에서 흡수될 가능성이 높습니다. (식사 상태: {state}, 위장 pH: {ph:.2f})")
